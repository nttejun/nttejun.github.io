...
| Tool | bower |
| 내용 | 사회 이슈가 등장하면 해당 이슈에 대한 생각과 정보를 서로 공유할 수 있는 서비스입니다.<br> 브라우저 **요청을 처리하는 Servlet, JDBC, MySQL 데이터처리를 구현하며 WAS 작동원리**에 대한 학습했습니다. 기본기능으로 로그인, 이슈등록, 이슈보기를 구현했습니다. 이슈를 등록하면 데이터가 바로 반영될 수 있도록 이슈보기를 Ajax를 사용해 비동기로 구현했습니다. **이슈보기 데이터 요청처리는 JSON 형태로 데이터 파싱**했습니다. AWS EC2 윈도우에서 리눅스 인스턴스로 접속하고 리눅스 웹서버를 생성했습니다. **부트스트랩 디자인 프레임워크를 사용**하여 반응형 웹을 구현했습니다. 부트스트랩, 제이쿼리 등 라이브러리를 관리하기 위해 **Bower 패키지 관리도구를 사용**했습니다.|
